// Script to fix Amazon links with real working ASINs
...
async function fixAmazonLinksWithRealAsins() {
  try {
    console.log('🚀 Fixing Amazon links with real working ASINs...');
    
    // Add products with unique titles to avoid conflicts
    const productsWithUniqueIds = realAmazonProducts.map((product, index) => ({
      ...product,
      title: `${product.title} - Real ASIN ${index + 1}`
    }));
    ...
    if (result.success) {
      ...
      console.log('🎯 All products now have real Amazon ASINs with 50%+ discounts!');
